...
class ReviewSerializerTests(TestCase):
    def test_paid_purchase_marks_review_as_buyer(self):
        ...
        gym = Gym.objects.create(owner=owner, name='Review Gym', latitude=35.0, longitude=51.0)
        group = GroupPackage.objects.create(title='Monthly')
        package = Package.objects.create(group_package=group, gym=gym, title='Basic', gender='male', price=Decimal('100.00'), duration=30)
